<?php

namespace CoreBundle\Entity;

use Doctrine\ORM\Mapping as ORM;
use Doctrine\Common\Collections\ArrayCollection;

/**
 * CoreBundle\Entity\Categorias
 *
 * @ORM\Entity()
 * @ORM\Table(name="categorias", indexes={@ORM\Index(name="fk_categorias_segmentos1_idx", columns={"idSegmento"})})
 */
class Categorias
{
    /**
     * @ORM\Id
     * @ORM\Column(type="integer")
     * @ORM\GeneratedValue(strategy="AUTO")
     */
    protected $id;

    /**
     * @ORM\Column(type="string", length=255, nullable=true)
     */
    protected $nombre;

    /**
     * @ORM\Column(type="string", length=10, nullable=true)
     */
    protected $clave;

    /**
     * @ORM\Column(type="boolean", nullable=true)
     */
    protected $activo;

    /**
     * @ORM\OneToMany(targetEntity="Clientes", mappedBy="categorias")
     * @ORM\JoinColumn(name="id", referencedColumnName="idCategoria", nullable=false)
     */
    protected $clientes;

    /**
     * @ORM\ManyToOne(targetEntity="Segmentos", inversedBy="categorias")
     * @ORM\JoinColumn(name="idSegmento", referencedColumnName="id", nullable=false)
     */
    protected $segmentos;

    public function __construct()
    {
        $this->clientes = new ArrayCollection();
    }

    public function __toString()
    {
        return $this->nombre;
    }

    /**
     * Set the value of id.
     *
     * @param integer $id
     * @return \CoreBundle\Entity\Categorias
     */
    public function setId($id)
    {
        $this->id = $id;

        return $this;
    }

    /**
     * Get the value of id.
     *
     * @return integer
     */
    public function getId()
    {
        return $this->id;
    }

    /**
     * Set the value of nombre.
     *
     * @param string $nombre
     * @return \CoreBundle\Entity\Categorias
     */
    public function setNombre($nombre)
    {
        $this->nombre = $nombre;

        return $this;
    }

    /**
     * Get the value of nombre.
     *
     * @return string
     */
    public function getNombre()
    {
        return $this->nombre;
    }

    /**
     * Set the value of clave.
     *
     * @param string $clave
     * @return \CoreBundle\Entity\Categorias
     */
    public function setClave($clave)
    {
        $this->clave = $clave;

        return $this;
    }

    /**
     * Get the value of clave.
     *
     * @return string
     */
    public function getClave()
    {
        return $this->clave;
    }

    /**
     * Set the value of activo.
     *
     * @param boolean $activo
     * @return \CoreBundle\Entity\Categorias
     */
    public function setActivo($activo)
    {
        $this->activo = $activo;

        return $this;
    }

    /**
     * Get the value of activo.
     *
     * @return boolean
     */
    public function getActivo()
    {
        return $this->activo;
    }

    /**
     * Set the value of idSegmento.
     *
     * @param integer $idSegmento
     * @return \CoreBundle\Entity\Categorias
     */
    public function setIdSegmento($idSegmento)
    {
        $this->idSegmento = $idSegmento;

        return $this;
    }

    /**
     * Get the value of idSegmento.
     *
     * @return integer
     */
    public function getIdSegmento()
    {
        return $this->idSegmento;
    }

    /**
     * Add Clientes entity to collection (one to many).
     *
     * @param \CoreBundle\Entity\Clientes $clientes
     * @return \CoreBundle\Entity\Categorias
     */
    public function addClientes(Clientes $clientes)
    {
        $this->clientes[] = $clientes;

        return $this;
    }

    /**
     * Remove Clientes entity from collection (one to many).
     *
     * @param \CoreBundle\Entity\Clientes $clientes
     * @return \CoreBundle\Entity\Categorias
     */
    public function removeClientes(Clientes $clientes)
    {
        $this->clientes->removeElement($clientes);

        return $this;
    }

    /**
     * Get Clientes entity collection (one to many).
     *
     * @return \Doctrine\Common\Collections\Collection
     */
    public function getClientes()
    {
        return $this->clientes;
    }

    /**
     * Set Segmentos entity (many to one).
     *
     * @param \CoreBundle\Entity\Segmentos $segmentos
     * @return \CoreBundle\Entity\Categorias
     */
    public function setSegmentos(Segmentos $segmentos = null)
    {
        $this->segmentos = $segmentos;

        return $this;
    }

    /**
     * Get Segmentos entity (many to one).
     *
     * @return \CoreBundle\Entity\Segmentos
     */
    public function getSegmentos()
    {
        return $this->segmentos;
    }

    public function __sleep()
    {
        return array('id', 'nombre', 'clave', 'activo', 'idSegmento');
    }
}